...
const isNetlifyPreview = (origin) =>
  /^https:\/\/[a-z0-9-]+--schoolfonds\.netlify\.app$/.test(origin);
...
app.use((req, res, next) => {
  console.log(
    `[REQ] ${new Date().toISOString()} ${req.method} ${req.originalUrl} Origin=${req.headers.origin || "-"} Auth=${!!req.headers.authorization}`,
  );
  next();
});
...
try {
  ...
  console.log(
    "ℹ️ referralController yoq — /api/ref/* ruteni otkazib yuborildi",
  );
}
        ...
        try {
          ...
          app._router.stack.forEach((mw) => {
            if (mw.route && mw.route.path) {
              out.push(
                `${Object.keys(mw.route.methods).join(",").toUpperCase()} ${mw.route.path}`,
              );
...
app.use((req, res) => {
  res
    .status(404)
    .json({ error: `Route topilmadi: ${req.method} ${req.originalUrl}` });
});
