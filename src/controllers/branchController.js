...
exports.createBranch = async (req, res) => {
  try {
    ...
    if (!name?.trim())
      return res.status(400).json({ success: false, error: 'Filial nomi majburiy' })
    ...
    const count = await Branch.countDocuments({ teacher: teacherId, isActive: true })
    ...
    res.status(201).json({ success: true, message: 'Filial yaratildi', branch })
  } catch (e) {
...
exports.getBranches = async (req, res) => {
  try {
    const teacherId = req.user.id
    const branches  = await Branch.find({ teacher: teacherId, isActive: true }).sort({ createdAt: 1 })
    ...
    const branchStats = await Promise.all(branches.map(async (b) => {
      ...
      const payments = await MonthlyPayment.find({
        class: { $in: classIds }, month: currentMonth, year: currentYear,
      })
      ...
      const totalInitial   = classes.reduce((s, c) => s + (c.initialBalance || 0), 0)

      const allExpenses    = await Expense.find({ teacher: teacherId, class: { $in: classIds } })
    ...
    if (unassigned.length > 0) {
      const sc = await Student.countDocuments({ class: { $in: unassignedIds } })
      unassignedStats = { classCount: unassigned.length, studentCount: sc }
    }

    res.json({ success: true, branches: branchStats, unassigned: unassignedStats })
  } catch (e) {
    res.status(500).json({ success: false, error: e.message })
  }
}
...
exports.assignClass = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: branchId ? 'Sinf filiallga biriktirildi' : 'Sinf filialsiz qilindi' })
  } catch (e) {
    res.status(500).json({ success: false, error: e.message })
  }
}
...
exports.deleteBranch = async (req, res) => {
  try {
    ...
    await Class.updateMany({ teacher: teacherId, branch: branchId }, { branch: null })

    branch.isActive = false
    await branch.save()

    res.json({ success: true, message: 'Filial o\'chirildi. Sinflar saqlab qolindi.' })
  } catch (e) {
    res.status(500).json({ success: false, error: e.message })
  }
}