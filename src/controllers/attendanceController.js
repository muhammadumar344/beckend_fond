...
// ── Davomat yaratish / yangilash (bulk) ─────────────────────
exports.saveAttendance = async (req, res) => {
  try {
    ...
    if (!classId || !date || !records?.length) {
      return res.status(400).json({ success: false, error: 'classId, date, records majburiy' })
    }

    // Sana validatsiya YYYY-MM-DD
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      return res.status(400).json({ success: false, error: 'date: YYYY-MM-DD formatida bo\'lsin' })
    }
...
exports.getDayAttendance = async (req, res) => {
  try {
    ...
    if (!date) return res.status(400).json({ success: false, error: 'date (YYYY-MM-DD) majburiy' })
    ...
    // Har bir o'quvchi uchun holat
    const attMap = {}
    attendance.forEach(a => { attMap[a.student.toString()] = a })

    const result = students.map(s => ({
      studentId:   s._id,
      studentName: s.name,
      rollNumber:  s.rollNumber,
      status:      attMap[s._id.toString()]?.status || null,  // null = belgilanmagan
    ...
    res.json({ success: true, date, class: { id: cls._id, name: cls.name }, students: result, summary })
  } catch (e) {
    res.status(500).json({ success: false, error: e.message })
  }
}
...
exports.getMonthlyStats = async (req, res) => {
  try {
    ...
    const students   = await Student.find({ class: classId }).sort({ rollNumber: 1 })
    const attendance = await Attendance.find({ class: classId, month: m, year: y })
    ...
    // Darslar o'tkazilgan kunlar soni
    const uniqueDates = [...new Set(attendance.map(a => a.date))]

    // Foiz hisoblash
    Object.values(statsMap).forEach(s => {
      s.percent = uniqueDates.length > 0
        ? Math.round(((s.present + s.late) / uniqueDates.length) * 100)
        : 0
    })

    const studentStats = Object.values(statsMap).sort((a, b) => a.rollNumber - b.rollNumber)
    ...
    res.json({
      ...
      summary: {
        ...
        absent:  absentCount,
        late:    lateCount,
        avgAttendance: totalRecords > 0 ? Math.round(((presentCount + lateCount) / totalRecords) * 100) : 0,
      },
